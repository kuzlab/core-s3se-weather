...
構想から実装完了まで、そして**動かし始めてから出た障害**まで、
はまったところを中心にまとめる。
コードは https://github.com/kuzlab/core-s3se-weather にある。

一通り完成して公開したあとに4つの障害が続けて出て、
**それが全部同じ根っこだった**というのが今回いちばん面白かったところ。
先を急ぐならそこ（「運用してから分かったこと」）だけ読んでもいい。
...
> **後日談:** この時点では「mbedtls を PSRAM に逃がす」で対処したが、
> それは対症療法だった。**内部 RAM が逼迫していた本当の理由**は数日後に
> 判明する（後述の #18）。同じ根から、さらに3つの障害が出た。
...
ここまでで一通り動き、GitHub に公開した。そのあと実際に電源を入れっぱなしにして
使い始めたら、**続けて4つの障害が出た。結果的に全部同じ根っこだった。**

### 15. 一晩置いたら固まっていた

翌朝、画面が前夜 23:15 のまま止まっていた。タップも効かない。

まず観測できた事実を並べる。

| 事実 | そこから言えること |
|---|---|
| `/dev/cu.usbmodem101` の作成時刻が **23:15** | そのとき USB が再列挙された＝リセットがあった |
| 以降その時刻が**更新されていない** | 23:15 以降は再起動していない |
| シリアル出力が**完全にゼロ**（10分以上） | ログを出すタスクが動いていない |
| バックライトは点いている | 電源は生きている |

パニックなら `PANIC_PRINT_REBOOT`（遅延0秒）で即再起動し、USB が再列挙されるはず。
それが起きていない。割り込み禁止のまま無限ループなら Interrupt WDT が
300ms で発火する。それも起きていない。

**つまりクラッシュではなく、タスクが待ち状態のまま進まなくなっている。**
この状態だとアイドルタスクは回り続けるので Task WDT も発火せず、
永久に固まったままになる。

#### JTAG で中を見る

推測を重ねても仕方がないので、デバッガを繋いだ。
**この基板は USB-Serial-JTAG なので追加ハードなしで JTAG が使える。**
...
OpenOCD はアタッチ時に halt するだけでリセットしないので、**状態は保存される。**
結果:

```
Thread 3 "taskLVGL":  State: Running @CPU1
Thread 9 "ui":
  #5  lvgl_port_lock (timeout_ms=0)          ← xTicksToWait = 4294967295
  #6  ui_set_summary (...) at ui.c:449
  #7  render () at main.c:306
```

CPU1 のプログラムカウンタを引くと決定的だった。

```
(gdb) info symbol 0x420184C4
wait_for_flushing + 40 in section .flash.text
```

LVGL のソースを見る。

```c
/* lv_refr.c:1444 wait_for_flushing() */
if(disp->flush_wait_cb) {
    ...
}
else {
    while(disp->flushing);   // ← タイムアウトも yield も無い
}
```

`disp->flushing` は **LCD への転送完了割り込み**から
`lv_disp_flush_ready()` が呼ばれてはじめてクリアされる。
その通知が1回でも失われると、**LVGL はここで永久に回り続ける。**
しかも LVGL タスクは **LVGL ロックを握ったまま**このループに入る。

連鎖はこうだった。

1. 転送完了通知が届かない
2. LVGL がロックを握ったままスピン
3. UI タスクが `lvgl_port_lock()` で永久ブロック → 画面更新が止まる
4. タッチも LVGL タスクが読むので効かなくなる
5. クラッシュではないのでウォッチドッグも再起動も起きない

**ハングしたら電源を切る前に JTAG で状態を取ること。** これが無ければ
「なんとなく固まる」で終わっていた。

#### ついでに見つけた実装ミス

```c
.max_transfer_sz = BSP_LCD_H_RES * LCD_FILL_ROWS * sizeof(uint16_t),  // = 10,240
```

**マイルストーン2の全画面塗りつぶし用に決めた値を、LVGL を入れたとき
見直していなかった。** 全画面フラッシュは 153,600 バイトなので、
esp_lcd がこれを15分割して転送していた。分割自体は正しく処理されるが、
1回の描画あたりの転送回数が15倍で、完了通知を落とす機会もその分多い。

### 16. 「じゅんびちゅう」から進まない

上の修正を入れて焼いたら、今度は起動時の表示から一切動かなくなった。
Wi-Fi にも繋がらない。またシリアルは無言。
...
**UI タスクとネットワークタスクが存在しない。**
`main` タスクも消えている（＝ `app_main` は最後まで走って正常終了した）。
つまり `xTaskCreate` は呼ばれ、**失敗した**。

```c
xTaskCreate(ui_task, "ui", 8192, NULL, UI_TASK_PRIO, &s_ui_task);   // 戻り値を見ていない
xTaskCreate(net_task, "net", 8192, NULL, NET_TASK_PRIO, &s_net_task);
xTaskCreate(stall_watchdog_task, "stall_wd", 3072, NULL, ...);      // これだけ成功
```

スタック 8192 の2本が失敗し、3072 の1本だけ成功していた。内部 RAM 不足である。

**`xTaskCreate` の戻り値を確認していなかったので、失敗が完全に不可視だった。**
アプリケーションの中核タスクが2本とも起動していないのに、
システムは何事もなかったように起動を完了していた。

さらに順序も悪かった。`app_main` は **`wifi_start()`（内部 RAM を数十KB消費）を
先に呼んでから**この2つのタスクを作っていた。

### 17. 再起動ループ

戻り値チェックを入れ、タスク生成を Wi-Fi より前に移した。焼いた。
**今度は起動して数秒で再起動、を延々と繰り返すようになった。**

これは**修正が意図通り働いた結果**でもある。黙って壊れていたものが、
失敗を検出して再起動するようになった。ただし内部 RAM 不足そのものは
解決していない。ログを見ると:

```
E (1870) wifi:Expected to init 10 rx buffer, actual is 4
E (1873) wifi_init: Failed to deinit Wi-Fi driver (0x3001)
```

`wifi_start()` の中の `ESP_ERROR_CHECK(esp_wifi_init(&init_cfg))` が
abort していた。

**`ESP_ERROR_CHECK` は、棚に置いて使う機器では凶器になりうる。**
再起動ループは最悪の故障モードで、所有者には何が起きているか分からず、
ログも取れない。全部除去して、失敗時は素直にエラーを返すようにした。
...
画面が出たまま「Wi-Fiにつながりません」と表示されるほうが、
所有者が対処できるぶん遥かにマシである。

### 18. 真犯人: 描画バッファは PSRAM 指定でも内部 RAM を食う

ここまで全部「内部 RAM が足りない」で説明がつく。
では**何が食っているのか**。推測をやめて、起動の各段階で実測することにした。
...
```
internal RAM at boot:       236,311 free, largest block 139,264
internal RAM after display: 232,599 free, largest block 139,264
internal RAM after LVGL:     71,119 free, largest block  31,744   ← 161,480 バイト消えた
internal RAM before tasks:   65,027 free
internal RAM after tasks:    44,187 free
→ E wifi: Expected to init 10 rx buffer, actual is 4
```

**LVGL の初期化だけで内部 RAM を 161KB 消費していた。**

ちなみに `max_transfer_sz` を全画面サイズに広げたコストは
`after display` の差分でわずか 3,712 バイト。**#15 で疑ったこれは無罪だった。**

#### なぜか

描画バッファは全画面ダブルバッファ（320x240x2 = 153,600 バイト x 2）で、
**`buff_spiram = true` を指定して PSRAM に置いていたつもりだった。**

バッファを40行分（320x40x2 = 25,600 バイト）に縮小して再測定すると:

```
internal RAM after lvgl_port_init: 225,555 free
internal RAM after add_disp:       199,891 free   ← 25,664 バイト
internal RAM after Wi-Fi:          120,567 free / 最大ブロック 47,104
```

**`add_disp` の消費が 25,664 バイト。描画バッファ1面分（25,600）とほぼ一致する。**
つまり `buff_spiram` を指定していても、**バッファサイズに比例した内部 RAM が
別途確保される。**

そもそも**LVGL は無効化された領域しか描画しない**ので、全画面バッファにしても
得るものは無かった。仕様書には「PSRAM に 8MB あるので余裕がある」と書いたし、
実際 PSRAM には余裕があった。**足りなかったのは内部 RAM のほうで、
そこを測っていなかった。**
...
| 地点 | 修正前 | 修正後 |
|---|---|---|
| LVGL 初期化後 | 71,119 | 199,871 |
| Wi-Fi 起動後 | （起動できず） | 120,567 |
| 最大連続ブロック | 31,744 | 47,104 |

**128,752 バイトの内部 RAM を取り戻した。**

TLS が2回目以降失敗した件（#4）で観測した最小値は 32,863 バイトだった。
**あの障害も、深夜のハングも、同じ内部 RAM 不足から来ていた可能性が高い。**
断定はできないが、条件は大きく改善した。

### 復帰できるようにする

根本原因が特定できても、**「もう起きない」と言い切れないものは
復帰手段を用意しておく**べきだと考えて、生存監視を入れた。

UI が5分間再描画できなければログを残して `esp_restart()` する。
他の全タスクが止まっても生き残る必要があるので、**依存を持たない専用タスク**に
した。ロックを取らず、I/O もせず、32bit のカウンタ1つしか触らない。

仕様書には「Wi-Fi切断時は再起動でごまかさない」と書いたが、これは Wi-Fi の話。
**真のデッドロックには他に復帰手段がない。** 電源を入れっぱなしで使う機器が、
人が抜き差しするまで固まったままというのが最悪の結果である。

---

## 時間帯によって「問い」を変える

一通り安定したあと、こういう指摘をもらった。

> いま10:19で「あめ！とりこんで」と出るけど「まだ干してないけど！」と
> 返答したくなっちゃう

**その通りだった。** 同じ予報でも、朝と夕方では知りたいことが違う。
まだ干していない朝10時に「取り込め」と言われても、何の役にも立たない。

時間帯で問いを切り替えるようにした。

| 時間帯 | 画面が答えている問い | 見る範囲 |
|---|---|---|
| 5〜11時 | きょう干せそうか | 今から夕方17時まで |
| 12〜19時 | 取り込むべきか（従来） | 前方24時間で最初に降る時刻まで |
| 20〜翌4時 | 次の日中は干せそうか | 次の 9〜17時 |

### 前方走査と窓判定は別のロジック

これは表示文言だけの話ではない。**「あと何時間で降るか」では
「午後がまるごと乾いているか」は分からない。**

そこで固定区間を評価する `judge_window()` を新設した。等級付けはこう。

- **窓の先頭から雨** → 最も強い警告（乾く隙がそもそも無い）
- 窓の1/3以上が雨 → 「きびしそう」
- 少しだけ雨 → 「びみょう」
- 雨なし → 「ほせそう」

夕方に1時間だけ降るのと、午後がまるごと雨なのを同じ扱いにしないため。
純粋関数のままなので、ホストでテストできる。

### 深夜2時の罠

**「次の日中」は、20時以降なら翌日だが、午前2時なら暦の上では同じ日。**

ここを取り違えると、夜中に見た人へ違う日の予報を出すことになる。
窓の計算も文言（「あした」/「きょう」）も切り替え、テストで固定した。
...
表示を直したあとで気づいたが、**ビープの発火条件が昼間モードのままだった。**
朝10時に「そろそろ取り込め」の遷移が起きたら鳴ってしまう。
まさに指摘された問題が音でも起きる。

ビープを12〜19時に限定した。判定の悪化自体はログには残す。
**深夜に「取り込め」のビープで起こされる事態も同時に防げた。**
...
| フォント（16px + 24px サブセット） | 約90文字 + ASCII |
| 内部 RAM 空き（Wi-Fi 起動後） | 120,567 バイト / 最大ブロック 47,104 |
| （修正前の同地点） | 71,119 バイト / 最大ブロック 31,744 |
| ホストテスト | 67 チェック |
...
1. **PSRAM 搭載機で `esp_get_free_heap_size()` だけを見ない。**
   合計値は 8MB の PSRAM に埋もれて内部 RAM の枯渇を完全に隠す。
   `heap_caps_get_free_size(MALLOC_CAP_INTERNAL)` と
   **`heap_caps_get_largest_free_block(MALLOC_CAP_INTERNAL)`** を併記する。
   確保が失敗するかは合計ではなく最大連続ブロックで決まる。
2. **「PSRAM に置いたから内部 RAM は大丈夫」を信じない。**
   `esp_lvgl_port` は `buff_spiram` 指定でもバッファサイズに比例した内部 RAM を
   確保していた。**指定を信じず実測する。**
3. **「余裕があるから」でサイズを決めない。** 全画面ダブルバッファは
   得るものが無いまま 161KB を占有していた。LVGL は無効化された領域しか描かない。
4. **起動の各段階でメモリを記録する。** 段階ごとの実測を入れるまで、
   4つの障害の共通原因にたどり着けなかった。

### 失敗の扱い

5. **戻り値を確認しない箇所が、いちばん静かに壊れる。**
   `xTaskCreate` の失敗を握りつぶしていたせいで、中核タスクが2本とも
   起動していないのにシステムは正常に起動を完了していた。
6. **`ESP_ERROR_CHECK` は棚に置く機器では凶器になりうる。**
   再起動ループは最悪の故障モードで、所有者には何も分からずログも取れない。
   画面が出たまま原因を表示するほうが遥かにマシ。
7. **原因が特定できないものには復帰手段を用意する。**
   デッドロックには他に手が無い。固まったままより再起動のほうがよい。
8. **外から区別がつかない失敗は、機器自身に説明させる。**
   「接続中」のまま止まる表示はハングと区別がつかない。

### 調査

9. **一次情報にあたる。** 電源シーケンスは推測で書けない。
   動作実績のあるコード（M5GFX / M5Unified / esp-bsp）が唯一の正解。
10. **診断ログが問題を隠していないか疑う。**
    「ヒープは 8,201,964 バイトで安定しています」という報告は、
    嘘ではなかったが、見るべき数字をひとつも見ていなかった。
11. **ハングしたら電源を切る前に JTAG で状態を取る。**
    ESP32-S3 は内蔵 USB-JTAG なので追加ハード不要。OpenOCD はアタッチ時に
    halt するだけでリセットしないので状態が保存される。
    これが無ければ「なんとなく固まる」で終わっていた。
12. **エラーメッセージを最後まで読む。** `multiple access on port?` と
    書いてあるのに、ハードウェアの問題だと思い込んで時間を溶かした。

### 設計

13. **人間の知覚に関わるパラメータを線形スケールにしない。**
    音量しかり、輝度しかり。
14. **同じデータでも、いつ見るかで知りたいことが変わる。**
    まだ干していない朝に「取り込め」と言う機器は使われない。
    表示だけでなく**音の発火条件も**同じ基準で揃える。
15. **段階的に実機確認する。** まとめて書いて最後に通電すると切り分けができない。

---

## 残っている宿題

- **LVGL の転送完了通知が失われる根本原因は未特定。** 内部 RAM を大幅に
  改善したので再発しない可能性が高いが、確証は無い。再発するなら
  LVGL の `flush_wait_cb` を差し替えて待ちにタイムアウトを設ける
  （無限ハングを1フレーム落ちに変える）のが次の手
...
- しきい値と時間帯の実運用での追い込み。「17時では遅い」「夜モードは21時から」
  などは使ってみないと分からない
