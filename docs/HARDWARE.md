...
> **⚠️ MCLK (GPIO0) をスピーカー再生に使ってはならない。**
>
> M5Unified が `pin_mck = GPIO_NUM_0` を設定しているのは**マイク（ES7210）の設定だけ**で、
> スピーカーの設定には `pin_mck` が無い（bck=34 / ws=33 / data_out=13 のみ）。
> AW88298 は BCK から動作するので MCLK は不要。
>
> そして **GPIO0 は ESP32-S3 のブートストラッピングピン**である。
> I2S ペリフェラルが GPIO0 を駆動している状態でリセットがかかると、
> **ROM ブートローダはアプリを起動せずダウンロードモードに分岐し、基板は
> USB を抜き差しするまで戻ってこない。**
>
> 実際にこれを踏んだ。生存監視が `esp_restart()` した直後に発生し、
> JTAG で見ると FreeRTOS のタスクが1つも無く CPU が ROM 領域を実行していた
...
マイルストーン1・2で実機（MAC `48:27:E2:7A:3B:B0`）から取得した実測値。
...
**SPEC §4.1 の6デバイスすべてが検出された。**

判明したこと:

- **CoreS3 SE にも ES7210（マイク）と AW88298（スピーカーアンプ）は実装されている。**
  §6 で「断定せず実測で確認する」としていた点が確定した。SPEC §1.4 のビープ音は
  ハードウェア的に実現可能。
- **FT6336U (0x38) と AW88298 (0x36) は電源シーケンス投入前には応答しない。**
  つまりこの2つは AXP2101 のレール投入と AW9523B のリセット解除に依存している。
  電源シーケンスを省略すると「タッチICが見つからない」形で失敗する。
- AXP2101 の投入前の `LDO_EN0` (reg 0x90) は `0x0b` だった。投入後は `0xBF`。
- SPIピンのプルアップ判定は `level_sum=3`（3本ともHIGH）。よって VBUS 5V出力は不要な
  ケースで、AW9523B には `0x02=0b101` / `0x03=0b00000011` を書いている。
...
**この個体は M5GFX の既知テーブルに載っていない。** VENDID が 0x11 ではないため、
M5GFX に同じコードを走らせても "version read failed" となり ILI9342C にフォールバックする。

なお CIPHER の 0x64 は **FT6336U のチップID そのもの**なので、タッチICが FT6336U で
あることはこの値で裏付けられている。読み出し自体は成功していて、単に M5GFX が知る
ロットと違うというだけ。

**実装は ILI9342C として扱い、それで正しく動作することを実機で確認済み**
（赤→緑→青→白→黒の全画面塗りつぶしが、指定どおりの色順で表示された）。
ILI9342E 用の追加初期化コマンド列は不要。

### 8-4. 動作の安定性

20秒間隔の診断ループで2分以上連続動作させ、空きヒープは 8,717,772 バイトから
1バイトも変動しなかった。PSRAM 8MB がヒープに組み込まれていることも確認
