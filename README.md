...
| `LW_NIGHT_START_HOUR` | 22 | 夜モード（減光＋ダークテーマ）の開始時刻 |
| `LW_NIGHT_END_HOUR` | 7 | 夜モードの終了時刻 |
| `LW_BRIGHTNESS_NIGHT` | 128 | 夜間のバックライト輝度 |

### 夜モード

22時〜翌7時は、バックライトを落とすと同時に**配色をダークに切り替える**。

**減光だけでは足りない。** バックライトは AXP2101 の DLDO1 で 2.5〜3.3V の
9段階しかなく、明るい配色は暗い部屋ではどれだけ絞っても眩しい。
**減光の大部分は配色側が担っている。**

背景は純黒ではなく暗いグレー（`#14171C`）にしてある。純黒だと色付きの
判定バナーが「画面で唯一光っているもの」になってしまい、色として読めなくなるため。
文字も白まで上げず `#B6BEC8` に抑えている。

`LW_BRIGHTNESS_NIGHT` の値と DLDO1 電圧の対応:
...
**下げすぎるとバックライトの点灯閾値を下回り、減光ではなく完全消灯になる。**
切り替え時に実際の電圧がログに出るので、そこで確認できる。

```
main: backlight -> 128 (DLDO1 2.9V): ESP_OK
ui: theme -> night
```
...
### ソフトウェアリセット後に起動せず ROM に落ちる（修正済み）

**症状:** 生存監視が `esp_restart()` したあと、アプリが起動しない。
画面はリセット前の内容を保持したまま、シリアルも無言。USB を抜き差しするまで戻らない。

**原因:** I2S の MCLK を **GPIO0** に割り当てていた。GPIO0 は ESP32-S3 の
ブートストラッピングピンで、リセット時に Low だと ROM はアプリを起動せず
ダウンロードモードに分岐する。

**そもそも MCLK は不要だった。** M5Unified が `pin_mck = GPIO_NUM_0` を設定するのは
マイクの設定だけで、スピーカー側には無い。AW88298 は BCK から動作する。

JTAG での確認:

```
Reset cause (3) - (Software core reset)
Thread 1 "Current Execution"  PC=0x40041a76   ← ROM 領域
（FreeRTOS のタスクが1つも存在しない）
```

**対処:**

1. `.mclk = I2S_GPIO_UNUSED`。GPIO0 には一切触れない
2. 全ての再起動経路を `restart_safely()` に通し、I2S とアンプを停止して
   **GPIO0 を入力に戻してから**リセットする
3. 生存監視が「一度も描画していない」場合に無効になっていた穴を塞いだ
   （起動直後に固まったケースで監視が効かなかった）

**教訓: 復旧機構そのものが状態を悪化させていないか確かめること。**
ストラッピングピンをペリフェラルに割り当てるときは、リセット経路への影響を必ず考える。

### 画面が真っ黒になり、再起動しないと戻らない

**症状:** しばらく動かしていると画面が完全に真っ黒になる。UI タスクもネットワークも
正常に動き続けており（ログは出ている、メモリも健全）、再起動すると直る。

**疑っている原因:** `axp2101` の電源レール制御レジスタ `0x90` に対する
read-modify-write。このレジスタは **ALDO1-4 / BLDO1-2 / DLDO1 の全レールの
ON/OFF を1バイトに持っている**。読み出しが I2C のグリッチで化けると、
化けた値をそのまま書き戻してしまう。

```c
/* 修正前 */
axp2101_read(h, 0x90, &cur);      /* ← ここで 0x00 が返ったら */
axp2101_write(h, 0x90, cur | 0x80);  /* ← 0x80 を書く = 他のレールが全部OFF */
```

この経路が走るのは**夜間減光の切り替え時（22時と6時）だけ**なので、
「しばらくすると」という症状と合う。失敗するとバックライト（DLDO1）だけが残り、
**LCD ロジックのレールが落ちて画面が真っ黒**になる。LVGL は何事もなく描画を
続けるため、システムからは異常に見えない。再起動するまで戻らない。

**対処:**

1. レジスタ `0x90` は**書いた値をシャドウとして保持し、読み戻さない**。
   1回の読み化けで他のレールを巻き込むことがなくなる
2. 60秒ごとに実レジスタを読み戻してシャドウと照合し、
   食い違っていればログを残して**書き戻す（自己修復）**
3. バックライト設定時に DLDO1 の電圧をログに出す。
   使える範囲は 2.5〜3.3V しかないので、「暗い」のか「消えている」のかを
   数値で切り分けられるようにした

正常時のログ:

```
main: PMIC: LDO_EN0=0xbf (expected 0xbf), DLDO1=28 (3.3V)
```

再発した場合:

```
axp2101: LDO_EN0 drifted: chip has 0x80, expected 0xbf -- restoring
bsp: backlight rail is off -- the screen is dark
```

**レジスタ化けが実際に起きたという証拠はまだ無い。** 上記は症状との符合による
推定で、診断ログはそれを確かめるために入れてある。
